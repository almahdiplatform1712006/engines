...
2. Draft: `npm run golden:draft -- <book>`. The current page reader (`GOLDEN_READER`: `stub`, the default, or `model`, Engines' own page reader on `AI_MODEL` through OpenRouter) reads each page and writes `truth/<page>.json` with `status: "draft"`. Existing truth is kept (`--force` replaces drafts, but never corrected truth). A page whose image is missing, whose truth file is broken, or that the reader fails on is reported as a failure with its reason, and the other pages carry on. It also writes `sheet.html`, which shows a broken truth file as it is, with the error.
