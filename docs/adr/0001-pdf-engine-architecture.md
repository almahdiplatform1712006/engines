...
| `src/offset/`    | `samplePages(count)`, `fitSegments(pairs)`, `pdfToPrinted(segments, pdf)` (pure)                                                                             | Digit normalisation, piecewise fit by mode, OCR outliers                                                                   |
| `src/reading/`   | `PageReader`: `readPage`, `readPair`, `readPrintedNumber`, `solve`, `readContents`. Two adapters: `modelReader` (Vercel AI SDK) and `scriptedReader` (tests) | Prompts, schemas, provider setup, `length` retries, usage and cost logging                                                 |
| `src/render/`    | `pageCount(file)`, `renderPage(file, page, out)`, `normalisePhoto`, `cropImage`                                                                              | poppler as a child process (`pdftoppm`, falling back to `pdftocairo`), sharp                                               |
| `src/storage/`   | `BlobStore`: `put`, `get`, `delete`, `signedUrl`, `createUpload`, `uploadStatus`. Adapters: local filesystem and Google Cloud Storage                        | Resumable upload protocol, URL signing                                                                                     |
...
| `src/documents/` | Document store: create, status transitions, pages, revisions, the §3 view                                                                                    | SQL                                                                                                                        |
| `src/jobs/`      | `registerPipeline(boss, deps)`; `Scheduler.admit()`                                                                                                          | Queue names, per-page tasks, retries, settling, sweeps, webhooks                                                           |
...
- `modelReader` is tested against the AI SDK's mock language model (schema rejection, `length` retry, usage logging).
